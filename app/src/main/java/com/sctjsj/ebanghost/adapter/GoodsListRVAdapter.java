...
import com.sctjsj.ebanghost.activity.BigPicActivity;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if(getItemViewType(position)==MENU_TYPE){
            ...
            if (dishholder != null) {
                ...
                Picasso.with(mContext).load(dish.getUrl()).resize(DpUtils.dpToPx(mContext,60f),DpUtils.dpToPx(mContext,60f)).error(R.mipmap.icon_portrait_load_failed).into(dishholder.ivIcon);
